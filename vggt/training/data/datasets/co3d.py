...
class Co3dDataset(BaseDataset):
    def __init__(
        ...
        for c in category:
            for split_name in split_name_list:
                ### suno: annotation 파일 경로 ex) apple_train.jgz
                ...
                for seq_name, seq_data in annotation.items():
                    ...
                    # --- [추가] 실제 이미지 파일이 있는지 확인하는 코드 시작 ---
                    # 시퀀스의 첫 번째 이미지 경로를 가져와서 존재하는지 확인
                    first_frame_path = seq_data[0]["filepath"]
                    full_path = osp.join(self.CO3D_DIR, first_frame_path)
    ...
    def get_data(
        ...
        if len(images) == 0:
            logging.warning(f"Sequence {seq_name} has no valid images. Retrying...")
